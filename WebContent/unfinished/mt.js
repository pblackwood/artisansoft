...
(function () {
  ...
  function _initializeVariables() {
    ...
    loadingIcon = "<img title='Loading...' src='http://artisansoft.com/mt-static/images/indicator.white.gif' alt='Loading' />";
  ...
  function _updateComments() {
    ...
    jsonUrl = [
        "http://artisansoft.com/cgi-sys/cgiwrap/artisnk1/managed-mt/mt-comments.cgi?__mode=comment_listing&direction=",
        direction,
        "&entry_id=",
        entryID,
        "&limit=",
...
function mtScore(entry_id) {
    ...
    var url = 'http://artisansoft.com/cgi-sys/cgiwrap/artisnk1/managed-mt/mt-cp.cgi';
...
function script_follow(id) {
    ...
    var xh = mtGetXmlHttp();
    if (!xh) return false;
    xh.open('POST', 'http://artisansoft.com/cgi-sys/cgiwrap/artisnk1/managed-mt/mt-cp.cgi', true);
    xh.onreadystatechange = function() {
        if ( xh.readyState == 4 ) {
            if (xh.status && ( xh.status != 200 ) ) {
                // error - ignore
    ...
    if (span) span.innerHTML = '<img src="http://artisansoft.com/mt-static/images/indicator.white.gif" height="10" width="10" alt="Following..." />';
}
...
function script_leave(id) {
    ...
    var span = document.getElementById('following-status');
    if (span) span.innerHTML = '<img src="http://artisansoft.com/mt-static/images/indicator.white.gif" height="10" width="10" alt="Leaving..." />';
}
...
var mtCookieDomain = ".artisansoft.com";
var mtCookiePath = "/";
...
function mtFetchUser(cb) {
    ...
    if ( ( cb == 'mtSetUser' ) && mtGetUser() ) {
        ...
        script.src = 'http://artisansoft.com/cgi-sys/cgiwrap/artisnk1/managed-mt/mt-comments.cgi?__mode=session_js&blog_id=1&jsonp=' + cb + '&ts=' + ts;
...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = 'e0c04138b017ec98528ea2d4eb5046619dc162bb';
...
function mtSignIn() {
    ...
    var url = 'http://artisansoft.com/cgi-sys/cgiwrap/artisnk1/managed-mt/mt-cp.cgi?__mode=login&blog_id=1';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
        } else {
            url += '&return_url=http%3A%2F%2Fartisansoft.com%2Funfinished%2F';
        }
    } else {
...
function mtSignOut(entry_id) {
    ...
    var doc_url = document.URL;
    doc_url = doc_url.replace(/#.+/, '');
    var url = 'http://artisansoft.com/cgi-sys/cgiwrap/artisnk1/managed-mt/mt-cp.cgi?__mode=logout&blog_id=1';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
...
function mtShowGreeting() {
    ...
    if ( u && u.is_authenticated ) {
        if ( u.is_banned ) {
            ...
            var user_link;
            if ( u.is_author ) {
                user_link = '<a href="http://artisansoft.com/cgi-sys/cgiwrap/artisnk1/managed-mt/mt-cp.cgi?__mode=edit&amp;return_to=' + encodeURIComponent(document.URL) + '&amp;blog_id=1';
                user_link += '">' + u.name + '</a>';
            } else {
